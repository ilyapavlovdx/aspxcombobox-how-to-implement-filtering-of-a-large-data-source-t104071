...
Partial Public Class _Default
	...
	Protected Sub ASPxComboBox_OnItemsRequestedByFilterCondition_SQL(ByVal source As Object, ByVal e As ListEditItemsRequestedByFilterConditionEventArgs)
		...
		SqlDataSource1.SelectCommand = "SELECT [ShipName], [ShipCity], [ShipRegion], [OrderID] FROM " & ControlChars.CrLf & "                (select [ShipName], [ShipCity], [ShipRegion], [OrderID], row_number()over(order by t.[ShipName]) as [rn] from [Orders] as t " & ControlChars.CrLf & "                where (([ShipName] + ' ' + [ShipCity] + ' ' + [ShipRegion]) LIKE @filter)) as st where st.[rn] between @startIndex and @endIndex"
